...
# IDs del servidor y canal donde se publicarán los resultados
TARGET_GUILD_ID = 1126922814906179687
...
# --- Búsqueda con scraping flexible ---
...
async def buscar(interaction: discord.Interaction, termino: str):
    await interaction.response.defer(ephemeral=True)  # mensaje solo visible para quien usa el comando
    ...
    # Enviar los resultados solo al canal especificado
    guild = bot.get_guild(TARGET_GUILD_ID)
    if guild:
        canal = guild.get_channel(TARGET_CHANNEL_ID)
        if canal:
            await canal.send(
            ...
            await interaction.followup.send(f"✅ Resultados publicados en {canal.mention}.")
            return
