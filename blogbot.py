...
async def buscar_posts(keyword: str):
    ...
    for h2 in soup.find_all("h2", class_="entry-title"):
        ...
        # 1. Dentro del mismo contenedor
        img_tag = parent.find("img") if parent else None

        # 2. Si no, en un contenedor anterior (hermano)
        if not img_tag:
            prev = h2.find_previous_sibling()
            if prev:
                img_tag = prev.find("img") if prev else None

        # 3. Si aún no, buscar una imagen anterior en el documento (usualmente la miniatura)
        if not img_tag:
...
def crear_embed(post):
    ...
    if post.get("image"):
        embed.set_thumbnail(url=post["image"])  # ← miniatura
